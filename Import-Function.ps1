function Import-Function {
    ...
    process {
        try {
            ...
            $resourcesPath = [System.IO.Path]::Combine($PSScriptRoot, 'src', 'resources')

            $functionFiles = Get-ChildItem -Path $privatePath, $publicPath -Filter '*.ps1' -File

            # Create temp directory for flattened module
            $tempDir = [System.IO.Path]::Combine([System.IO.Path]::GetTempPath(), 'ModuleAssemblerTest')
            ...
            $functionFiles | Copy-Item -Destination $tempDir -Force
            ...
            $command = "Set-Location '$PSScriptRoot'; Write-Host 'Flattened module loaded at: $tempDir'; " + ($importCommands -join '; ')
